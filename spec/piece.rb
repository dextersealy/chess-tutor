  ...
  describe "valid_moves" do
    context "pawn" do
      it "has two moves when on home row" do
        expect(board[[1, 0]].valid_moves).to match_array([[2, 0], [3, 0]])
        expect(board[[6, 0]].valid_moves).to match_array([[5, 0], [4, 0]])
      end
      it "has one move when off home row" do
        board = Board.new("4K3/8/P7/8/8/p7/8/4k3||")
        expect(board[[2, 0]].valid_moves).to match_array([[3, 0]])
        expect(board[[5, 0]].valid_moves).to match_array([[4, 0]])
      end
      it "cannot capture forward" do
        board = Board.new("4K3/8/P7/p7/8/8/8/4k3||")
        expect(board[[2, 0]].valid_moves).to match_array([])
        expect(board[[3, 0]].valid_moves).to match_array([])
      end
      it "can capture diagonally" do
        board = Board.new("4K3/8/P7/1p6/8/8/8/4k3||")
        expect(board[[2, 0]].valid_moves).to match_array([[3, 0], [3, 1]])
        expect(board[[3, 1]].valid_moves).to match_array([[2, 0], [2, 1]])
      end
    end
    ...
    context "rook" do
      it "can move horizontally and vertically" do
        ...
        expect(board[[3, 3]].valid_moves).to match_array(straight_line_moves(3, 3))
        expect(board[[5, 5]].valid_moves).to match_array(straight_line_moves(5, 5))
      end
    end
    ...
    context "bishop" do
      it "can move diagonally" do
        ...
        expect(board[[3, 3]].valid_moves).to match_array(diagonal_moves(3, 3))
        expect(board[[4, 3]].valid_moves).to match_array(diagonal_moves(4, 3))
      end
    end
    ...
    context "knight" do
      it "can do knight moves" do
        ...
        expect(board[[3, 3]].valid_moves).to match_array([
          [1, 2], [1, 4], [2, 5], [4, 5], [5, 4], [5, 2], [4, 1], [2, 1]
        ])
        expect(board[[4, 4]].valid_moves).to match_array([
    ...
    context "queen" do
      it "can move horizontally, vertically and diagonally" do
        ...
        expect(board[[2, 3]].valid_moves).to match_array(
          straight_line_moves(2, 3).concat(diagonal_moves(2, 3)))
        expect(board[[5, 5]].valid_moves).to match_array(
    ...
    context "king" do
      it "can move one step in any direction" do
        ...
        expect(board[[1, 4]].valid_moves).to match_array(single_step_moves(1, 4))
        expect(board[[6, 4]].valid_moves).to match_array(single_step_moves(6, 4))
      end
