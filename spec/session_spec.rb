...
describe Session do
  ...
  let(:cook) { {'_tracks_app' => { 'xyz' => 'abc' }.to_json} }
  ...
  describe '#store_session' do
    context 'without cookies in request' do
      ...
      it 'adds new cookie with \'_tracks_app\' name to response' do
        cookie_str = res.headers['Set-Cookie']
        cookie = Rack::Utils.parse_query(cookie_str)
        expect(cookie['_tracks_app']).not_to be_nil
      end

      it 'stores the cookie in json format' do
        cookie_str = res.headers['Set-Cookie']
        cookie = Rack::Utils.parse_query(cookie_str)
        cookie_val = cookie['_tracks_app']
    ...
    context 'with cookies in request' do
      before(:each) do
        cook = {'_tracks_app' => { 'pho' => 'soup' }.to_json }
        req.cookies.merge!(cook)
      end
